...
See the **`canvas`** example for the full bar chart, and the **`particles`** example for the
continuous-ticker side.
...
For open-ended motion (no target), override `update(dt)` and integrate by time — a particle
emitter registers a continuous animator and advances its array each frame:
