...
| `math` | ✅ done | [en](docs/en/math.md) · [es](docs/es/math.md) — interpolation, trig/waves, point generators, `[x,y]` vectors, seeded randomness |
...
| `svg` | ✅ done | [en](docs/en/svg.md) · [es](docs/es/svg.md) — SVG as a third projection: `svg()` namespaced factory + `toPoints`/`toPath`; model-driven SVG is just `CollectionView` |
| `canvas` | 🧪 experimental | [en](docs/en/canvas.md) · [es](docs/es/canvas.md) — render the same Model/Collection to canvas (`Stage`/`Node2D`/`CanvasLayer`, `Rect`/`Circle`/`Line`/`Path`/`Polygon`/`Arc`); import from `lumenjs/canvas` |
